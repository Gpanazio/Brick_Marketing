...
const MARKETING_MODELS = {
    ...
    COPY_A:       { primary: { provider: 'openrouter', model: 'openai/gpt-5.1-codex' }, fallback: { provider: 'google', model: 'gemini-2.0-flash' } },
    ...
    COPY_SENIOR:  { primary: { provider: 'openrouter', model: 'openai/gpt-5.1-codex' }, fallback: { provider: 'openrouter', model: 'deepseek/deepseek-v3.2' } },
    WALL:         { primary: { provider: 'openrouter', model: 'anthropic/claude-opus-4.6' }, fallback: { provider: 'openrouter', model: 'openai/gpt-5.1-codex' } }
};
...
class PipelineRunner {
    ...
    async callWithRetry(systemPrompt, userPrompt, options = {}) {
        ...
        const MODEL_MAP = { ideias: IDEIAS_MODELS, marketing: MARKETING_MODELS };
    ...
    async runProjetos(briefingContent) {
        ...
        for (let loop = 1; loop <= MAX_PROJ_LOOPS; loop++) {
            ...
            try {
                ...
                const { raw } = await this.callWithRetry(executionRole, execPrompt, { expectJSON: true });
